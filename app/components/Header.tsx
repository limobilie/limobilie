...
export default function Header({ isDashboard = false }) {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      setIsLoggedIn(!!session)
      // Sécurité : si la session est détruite (déconnexion), on redirige vers l'accueil
      if (!session) {
        router.push('/')
      }
    })
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      setEmail('')
      setPassword('')
      // Optionnel : rediriger vers dashboard après login réussi
  ...
  const handleLogout = async () => {
    ...
    setMenuOpen(false)
    router.push('/') // Redirection immédiate vers l'accueil
  }

  return (
    <header className={`header ${scrolled || isDashboard ? 'scrolled' : ''} ${isDashboard ? 'dashboard-mode' : ''}`}>
      
      {/* MENU HAMBURGER (Sera rouge sur Dashboard grâce au CSS précédent) */}
        ...
        <ul className="menu-list">
          {isLoggedIn ? (
            <>
              ...
              <li onClick={handleLogout} className="logout-btn">🚪 Déconnexion</li>
            </>
          ) : (
            <>
              <li><Link href="/" onClick={() => setMenuOpen(false)}>Accueil</Link></li>
